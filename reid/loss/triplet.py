...
class TripletLoss(nn.Module):
	...
	def __init__(self, margin, normalize_feature=False):
		...
		self.margin_loss = nn.MarginRankingLoss(margin=margin).cuda()  # loss(x1, x2, y)=max(0, -y*(x1-x2)+m) 
	...
	def forward(self, emb, label):  
		...
		N = mat_dist.size(0)  # bs
		mat_sim = label.expand(N, N).eq(label.expand(N, N).t()).float() 

		dist_ap, dist_an = _batch_hard(mat_dist, mat_sim)  # [bs,1]
